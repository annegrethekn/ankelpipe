...
resource "azurerm_virtual_desktop_host_pool" "ffihostpool" {
...
# Create AVD DAG
resource "azurerm_virtual_desktop_application_group" "desktopponline" {
  provider            = azurerm
  resource_group_name = azurerm_resource_group.rg.name
  host_pool_id        = azurerm_virtual_desktop_host_pool.ffihostpool.id
  location            = azurerm_resource_group.rg.location
  type                = "Desktop"
  name                = "ffi-test-${var.prefix}-online"
  friendly_name       = "AVD Full Desktop"
  description         = "AVD Full Desktop"
  depends_on = [azurerm_virtual_desktop_host_pool.ffihostpool,
...
resource "azurerm_virtual_desktop_workspace_application_group_association" "ffionline" {
  provider             = azurerm
  application_group_id = azurerm_virtual_desktop_application_group.desktopponline.id
...
resource "azurerm_virtual_desktop_host_pool_registration_info" "avd_token" {
  hostpool_id     = azurerm_virtual_desktop_host_pool.ffihostpool.id
